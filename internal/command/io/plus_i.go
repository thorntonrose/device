...
//
// a: operation (0 | 1 | 5, default: 0); 0 = send transmit buffer, 1 = send transmit buffer with newline, 5 = wait for
// n characters then append to receive buffer
//
...
func NewPlusI(memory *mem.Memory) PlusI {
	return PlusI{command.New(memory)}
}
...
func (self PlusI) Run(parameters []string) (skip int) {
	...
	return etc.If(a == 5, func() int { return self.Receive(t, s, n) }, func() int { return self.Transmit(a) })()
}

func (self PlusI) Transmit(a int) int {
	data := string(*self.Memory.Buffers[mem.Transmit])
	log.Printf("PlusI.Transmit: %v\n", data)
	fmt.Print(data + etc.If(a == 1, "\n", ""))
