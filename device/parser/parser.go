...
func (self Parser) Parse(program string) (data map[int][]byte) {
	log.Println("Parse")
	data = map[int][]byte{}
	self.ParseLines(data, strings.Split(strings.TrimSpace(program), "\n"), 0)
...
func (self Parser) DataDirective(line string, sep string) (int, []byte) {
	if tokens := strings.SplitN(line, sep, 2); len(tokens) == 2 {
		return self.SlotNum(line, tokens[0]), self.Text(tokens[1])
	}

	return 0, nil
}

func (self Parser) SlotNum(line, token string) int {
